    ...
    .navbar {
      ...
      border-radius: 0;
		...
		<div id="page-wrapper" style="background-image: url(code/img/body-bg.png);background-color: #eef1f4;">
            <br><div class="row panel panel-default" >
                <div class="col-lg-12 panel-body" style="background-color: #1c1f25;">
				{!! Form::open(array('action' =>'TimecardController@clientall')) !!}
		<input type="hidden" name="_token" value="<?=  csrf_token(); ?>">
                    <b style="border-right: 2px solid #c7c9ce; font-weight:normal; font-size:25px; color:white;">Timecard&nbsp;&nbsp;</b>&nbsp;&nbsp;
					 <input type="text" class="form-control1" name="daterange4" placeholder="Select Date" value="{{ Input::get('daterange4')}}" 
	  ...
	  {!! Form::close() !!}
			...
			<table class="table  table-striped">
								...
								$dlsc1=$aec['updated_at'];
								$dlsd=date('Y-m-d',strtotime($dlsc1));
								
								$cdur=$aec['duration'];
								$cstatus=$aec['status'];
										 
										if(($dlsd >= $dateclient ) && ($dlsd <= $datecli))
										{
